...
int usage(int ret) {
  if ((! GetConsoleWindow() || ! GetStdHandle(STD_OUTPUT_HANDLE)) && GetProcessWindowStation()) popup_message(0, MB_OK, TSSM_MESSAGE_USAGE, TSSM_VERSION, TSSM_CONFIGURATION, TSSM_DATE);
  else print_message(stderr, TSSM_MESSAGE_USAGE, TSSM_VERSION, TSSM_CONFIGURATION, TSSM_DATE);
  return(ret);
}
...
static int elevate(int argc, TCHAR **argv, unsigned long message) {
  ...
  sei.lpVerb = _T("runas");
  sei.lpFile = (TCHAR *) tssm_imagepath();

  TCHAR *args = (TCHAR *) HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, EXE_LENGTH * sizeof(TCHAR));
  if (! args) {
    print_message(stderr, TSSM_MESSAGE_OUT_OF_MEMORY, _T("GetCommandLine()"), _T("elevate()"));
    return 111;
  }

  /* Get command line, which includes the path to TSSM, and skip that part. */
...
int _tmain(int argc, TCHAR **argv) {
  ...
  if (argc > 1) {
    ...
    */
    if (is_version(argv[1])) {
      _tprintf(_T("%s %s %s %s\n"), TSSM, TSSM_VERSION, TSSM_CONFIGURATION, TSSM_DATE);
      tssm_exit(0);
    }
    if (str_equiv(argv[1], _T("start"))) tssm_exit(control_service(TSSM_SERVICE_CONTROL_START, argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("stop"))) tssm_exit(control_service(SERVICE_CONTROL_STOP, argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("restart"))) {
      int ret = control_service(SERVICE_CONTROL_STOP, argc - 2, argv + 2);
      if (ret) tssm_exit(ret);
      tssm_exit(control_service(TSSM_SERVICE_CONTROL_START, argc - 2, argv + 2));
    }
    if (str_equiv(argv[1], _T("pause"))) tssm_exit(control_service(SERVICE_CONTROL_PAUSE, argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("continue"))) tssm_exit(control_service(SERVICE_CONTROL_CONTINUE, argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("status"))) tssm_exit(control_service(SERVICE_CONTROL_INTERROGATE, argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("statuscode"))) tssm_exit(control_service(SERVICE_CONTROL_INTERROGATE, argc - 2, argv + 2, true));
    if (str_equiv(argv[1], _T("rotate"))) tssm_exit(control_service(TSSM_SERVICE_CONTROL_ROTATE, argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("install"))) {
      if (! is_admin) tssm_exit(elevate(argc, argv, TSSM_MESSAGE_NOT_ADMINISTRATOR_CANNOT_INSTALL));
      create_messages();
      tssm_exit(pre_install_service(argc - 2, argv + 2));
    }
    if (str_equiv(argv[1], _T("edit")) || str_equiv(argv[1], _T("get")) || str_equiv(argv[1], _T("set")) || str_equiv(argv[1], _T("reset")) || str_equiv(argv[1], _T("unset")) || str_equiv(argv[1], _T("dump"))) {
      ...
      if (ret == 3 && ! is_admin && argc == 3) tssm_exit(elevate(argc, argv, TSSM_MESSAGE_NOT_ADMINISTRATOR_CANNOT_EDIT));
      /* There might be a password here. */
      for (int i = 0; i < argc; i++) SecureZeroMemory(argv[i], _tcslen(argv[i]) * sizeof(TCHAR));
      tssm_exit(ret);
    }
    if (str_equiv(argv[1], _T("list"))) tssm_exit(list_tssm_services(argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("processes"))) tssm_exit(service_process_tree(argc - 2, argv + 2));
    if (str_equiv(argv[1], _T("remove"))) {
      if (! is_admin) tssm_exit(elevate(argc, argv, TSSM_MESSAGE_NOT_ADMINISTRATOR_CANNOT_REMOVE));
      tssm_exit(pre_remove_service(argc - 2, argv + 2));
    }
  }
  ...
  */
  if (! GetStdHandle(STD_INPUT_HANDLE)) {
    /* Start service magic */
    SERVICE_TABLE_ENTRY table[] = { { TSSM, service_main }, { 0, 0 } };
    if (! StartServiceCtrlDispatcher(table)) {
      unsigned long error = GetLastError();
      /* User probably ran tssm with no argument */
      if (error == ERROR_FAILED_SERVICE_CONTROLLER_CONNECT) tssm_exit(usage(1));
      log_event(EVENTLOG_ERROR_TYPE, TSSM_EVENT_DISPATCHER_FAILED, error_string(error), 0);
      tssm_exit(100);
    }
  }
  else tssm_exit(usage(1));
