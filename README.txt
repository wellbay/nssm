...
TSSM is a service helper program similar to srvany and cygrunsrv.  It can 
start any application as an NT service and will restart the service if it 
fails for any reason.

TSSM also has a graphical service installer and remover.
...
Since version 2.1, TSSM can be compiled for x64 platforms.
Thanks Benjamin Mayrargue.

Since version 2.2, TSSM can be configured to take different actions
based on the exit code of the managed application.

Since version 2.3, TSSM logs to the Windows event log more elegantly.

Since version 2.5, TSSM respects environment variables in its parameters.

Since version 2.8, TSSM tries harder to shut down the managed application
gracefully and throttles restart attempts if the application doesn't run
for a minimum amount of time.

Since version 2.11, TSSM respects srvany's AppEnvironment parameter.

Since version 2.13, TSSM is translated into French.
Thanks François-Régis Tardy.

Since version 2.15, TSSM is translated into Italian.
Thanks Riccardo Gusmeroli.

Since version 2.17, TSSM can try to shut down console applications by
...
Since version 2.17, TSSM can redirect the managed application's I/O streams
to an arbitrary path.

Since version 2.18, TSSM can be configured to wait a user-specified amount
...
Since version 2.19, TSSM can add to the service's environment by setting
AppEnvironmentExtra in place of or in addition to the srvany-compatible
AppEnvironment.

Since version 2.22, TSSM can set the managed application's process priority
and CPU affinity.

Since version 2.22, TSSM can apply an unconditional delay before restarting
an application which has exited.

Since version 2.22, TSSM can rotate existing output files when redirecting I/O.

Since version 2.22, TSSM can set service display name, description, startup
type, log on details and dependencies.

Since version 2.22, TSSM can manage existing services.

Since version 2.25, TSSM can execute commands in response to service events.

Since version 2.25, TSSM can list services it manages.

Since version 2.25, TSSM can dump the configuration of services it manages.

Since version 2.25, TSSM can show the processes managed by a service.
...
properties such as startup method and desktop interaction.  TSSM may 
...
To install a service, run
    ...
    tssm install <servicename> <application> [<options>]

TSSM will then attempt to install a service which runs the named application 
...
--------------------
TSSM will launch the application listed in the registry when you send it a 
start signal and will terminate it when you send a stop signal.  So far, so 
much like srvany.  But TSSM is the Non-Sucking service manager and can take 
action if/when the application dies.

With no configuration from you, TSSM will try to restart itself if it notices
that the application died but you didn't send it a stop signal.  TSSM will
keep trying, pausing between each attempt, until the service is successfully
started or you send it a stop signal.

TSSM will pause an increasingly longer time between subsequent restart attempts
...
TSSM and it will retry within a few seconds.

By default, TSSM defines "a timely manner" to be within 1500 milliseconds.
...
Alternatively, TSSM can pause for a configurable amount of time before
attempting to restart the application even if it successfully ran for the
amount of time specified by AppThrottle.  TSSM will consult the REG_DWORD value
...
throttling, TSSM will pause the service for whichever is longer of the
...
TSSM will look in the registry under
...
If the application exited with code 1, for instance, TSSM will look for a
...
for the application by consulting the system event log.  TSSM will log the
exit code when the application exits.

Based on the data found in the registry, TSSM will take one of three actions:

If the value data is "Restart" TSSM will try to restart the application as
described above.  This is its default behaviour.

If the value data is "Ignore" TSSM will not try to restart the application
...
If the value data is "Exit" TSSM will exit gracefully.  The Windows Services
...
If the value data is "Suicide" TSSM will simulate a crash and exit without
...
that if the monitored application exits with code 0, TSSM will only honour a
request to suicide if you explicitly configure a registry key for exit code 0.
If only the default action is set to Suicide TSSM will instead exit gracefully.


Application priority
--------------------
TSSM can set the priority class of the managed application.  TSSM will look in
...
Processor affinity
------------------
TSSM can set the CPU affinity of the managed application.  TSSM will look in
the registry under HKLM\SYSTEM\CurrentControlSet\Services\<service>\Parameters
...
If AppAffinity is missing or invalid, TSSM will not attempt to restrict the
application to specific CPUs.

Note that the 64-bit version of TSSM can configure a maximum of 64 CPUs in this
...
--------------------
When stopping a service TSSM will attempt several different methods of killing
the monitored application, each of which can be disabled if necessary.

First TSSM will attempt to generate a Control-C event and send it to the
...
Secondly TSSM will enumerate all windows created by the application and send
them a WM_CLOSE message, requesting a graceful exit.

Thirdly TSSM will enumerate all threads created by the application and send
...
Finally TSSM will call TerminateProcess() to request that the operating
...
Any or all of the methods above may be disabled.  TSSM will look for the
...
to 5 and TSSM would not attempt to use those methods to stop the application.

Take great care when including 8 in the value of AppStopMethodSkip.  If TSSM
does not call TerminateProcess() it is possible that the application will not
exit when the service stops.

By default TSSM will allow processes 1500ms to respond to each of the methods
...
Console window
--------------
By default, TSSM will create a console window so that applications which
...
I/O redirection
---------------
TSSM can redirect the managed application's I/O to any path capable of being
...
File rotation
-------------
When using I/O redirection, TSSM can rotate existing output files prior to
...
example, the file tssm.log might be rotated to tssm-20131221T113939.457.log.
...
If AppRotateDelay is non-zero, TSSM will pause for the given number of
...
TSSM to rotate files which are held open by other processes, preventing the
...
They will be rotated regardless of whether TSSM would otherwise have appended
or replaced them.

TSSM can also rotate files which hit the configured size threshold while the
...
Note that online rotation requires TSSM to intercept the application's I/O
...
Timestamping output
-------------------
When redirecting output, TSSM can prefix each line of output with a
...
---------------------
TSSM can replace or append to the managed application's environment.  Two
...
As of version 2.25, TSSM parses AppEnvironment and AppEnvironmentExtra
...
As of version 2.25, TSSM can get and set the Environment block using
commands similar to:

    tssm get <servicename> Environment

It is worth reiterating that the Environment block is available to all
Windows services, not just TSSM services.


Service startup environment
---------------------------
The environment TSSM passes to the application depends on how various
...
Event hooks
-----------
TSSM can run user-configurable commands in response to application events.
These commands are referred to as "hooks" below.

All hooks are optional.  Any hooks which are run will be launched with the
environment configured for the service.  TSSM will place additional
...
synchronously.  TSSM will wait for these hooks to complete before continuing
...
or not.
   ...
   *Action: Pre - Called before TSSM attempts to launch the application.
    Action: Post - Called after the application successfully starts.

  Event: Stop - Triggered when the service is requested to stop.
   *Action: Pre - Called before TSSM attempts to kill the application.

  Event: Exit - Triggered when the application exits.
   *Action: Post - Called after TSSM has cleaned up the application.

  Event: Rotate - Triggered when online log rotation is requested.
   *Action: Pre - Called before TSSM rotates logs.
    Action: Post - Called after TSSM rotates logs.
...
TSSM sets the environment variable TSSM_HOOK_VERSION to a positive number.
Hooks can check the value of the number to determine which other environment
variables are available to them.

If TSSM_HOOK_VERSION is 1 or greater, these variables are provided:

  TSSM_EXE - Path to TSSM itself.
  TSSM_CONFIGURATION - Build information for the TSSM executable,
    eg 64-bit debug.
  TSSM_VERSION - Version of the TSSM executable.
  TSSM_BUILD_DATE - Build date of TSSM.
  TSSM_PID - Process ID of the running TSSM executable.
  TSSM_DEADLINE - Deadline number of milliseconds after which TSSM will
    kill the hook if it is still running.
  TSSM_SERVICE_NAME - Name of the service controlled by TSSM.
  TSSM_SERVICE_DISPLAYNAME - Display name of the service.
  TSSM_COMMAND_LINE - Command line used to launch the application.
  TSSM_APPLICATION_PID - Process ID of the primary application process.
    May be blank if the process is not running.
  TSSM_EVENT - Event class triggering the hook.
  TSSM_ACTION - Event action triggering the hook.
  TSSM_TRIGGER - Service control triggering the hook.  May be blank if
    the hook was not triggered by a service control, eg Exit/Post.
  TSSM_LAST_CONTROL - Last service control handled by TSSM.
  TSSM_START_REQUESTED_COUNT - Number of times the application was
    requested to start.
  TSSM_START_COUNT - Number of times the application successfully started.
  TSSM_THROTTLE_COUNT - Number of times the application ran for less than
    the throttle period.  Reset to zero on successful start or when the
    service is explicitly unpaused.
  TSSM_EXIT_COUNT - Number of times the application exited.
  TSSM_EXITCODE - Exit code of the application.  May be blank if the
    application is still running or has not started yet.
  TSSM_RUNTIME - Number of milliseconds for which the TSSM executable has
    been running.
  TSSM_APPLICATION_RUNTIME - Number of milliseconds for which the
    application has been running since it was last started.  May be blank
    if the application has not been started yet.

Future versions of TSSM may provide more environment variables, in which
case TSSM_HOOK_VERSION will be set to a higher number.
...
resumes from standby by setting AppEvents\Power\Resume to:
    ...
    %TSSM_EXE% restart %TSSM_SERVICE_NAME%

To set a hook on the command line, use

    tssm set <servicename> AppEvents <event>/<action> <command>

Note that TSSM will abort the startup of the application if a Start/Pre hook
...
If the application crashes and is restarted by TSSM, the order might be:
...
If TSSM is redirecting stdout or stderr it can be configured to redirect
...
-------------------------------
TSSM can edit the settings of existing services with the same GUI that is
...
TSSM offers limited editing capabilities for services other than those which
run TSSM itself.  When TSSM is asked to edit a service which does not have
...
----------------------------------------
TSSM can retrieve or set individual service parameters from the command line.
In general the syntax is as follows, though see below for exceptions.

    tssm get <servicename> <parameter>

    tssm set <servicename> <parameter> <value>

Parameters can also be reset to their default values.

    tssm reset <servicename> <parameter>

The parameter names recognised by TSSM are the same as the registry entry
names described above, eg AppDirectory.

TSSM offers limited editing capabilities for Services other than those which
run TSSM itself.  The parameters recognised are as follows:
...
Note that TSSM will concatenate all arguments passed on the command line
with spaces to form the value to set.  Thus the following two invocations
would have the same effect.

    tssm set <servicename> Description "TSSM managed service"
...
for a service
    ...
    tssm get <servicename> AppEnvironmentExtra
...
if it is not configured.
    ...
    tssm get <servicename> AppEnvironmentExtra CLASSPATH
...
KEY=VALUE pair in separate command line arguments.  For example:
    ...
    tssm set <servicename> AppEnvironment CLASSPATH=C:\Classes TEMP=C:\Temp
...
The following syntax would not remove TEMP=C:\Temp
    ...
    tssm set <servicename> AppEnvironment -TEMP=C:\Work\Temporary
...
separate command line arguments.  For example:
    ...
    tssm set <servicename> DependOnService RpcSs LanmanWorkstation
...
The following two lines set dependencies on RpcSs and LanmanWorkstation:
    ...
    tssm set <servicename> DependOnService RpcSs
    tssm set <servicename> DependOnService +LanmanWorkstation
...
NetBIOSGroup:
    ...
    tssm set <servicename> DependOnGroup NetBIOSGroup
    tssm set <servicename> DependOnGroup :NetBIOSGroup
    tssm set <servicename> DependOnGroup :+NetBIOSGroup
...
To retrieve the username, run
    ...
    tssm get <servicename> ObjectName

To set the username and password, run

    tssm set <servicename> ObjectName <username> <password>
...
invocation is valid and will have the expected effect.
    ...
    tssm set <servicename> ObjectName <username> correct horse battery staple
...
to Vista.  TSSM will set the service to automatic startup if delayed start is
unavailable.


The Type parameter is used to query or set the service type.  TSSM recognises
...
is in two stages as follows.
    ...
    tssm reset <servicename> ObjectName
    tssm set <servicename> Type SERVICE_INTERACTIVE_PROCESS


Controlling services using the command line
-------------------------------------------
TSSM offers rudimentary service control features.
...
The output of "tssm status" and "tssm statuscode" is a string
representing the service state, eg SERVICE_RUNNING.

The exit code of "tssm status" will be 0 if the status was
succesfully retrieved.  If the exit code is not zero there was
an error.

The exit code of "tssm statuscode" will be the numeric value
...
-------------------------------
TSSM can also remove services.  Run
...
Logging
-------
TSSM logs to the Windows event log.  It registers itself as an event log source
...
Because of the way TSSM registers itself you should be aware that you may not
be able to replace the TSSM binary if you have the event viewer open and that
running multiple instances of TSSM from different locations may be confusing if
they are not all the same version.


Listing managed services
------------------------
The following command will print the names of all services managed by TSSM:

    tssm list

To see all services on the system, not just TSSM's, use list all:
...
Note that if 32-bit TSSM is run on a 64-bit system running an older version of
Windows than Vista it will not be able to query the paths of 64-bit processes.


Exporting service configuration
-------------------------------
TSSM can dump commands which would recreate the configuration of a service.
...
quoted or escaped from the command prompt, TSSM tries hard to produce
...
To install an Unreal Tournament server:
    ...
    tssm install UT2004 c:\games\ut2004\system\ucc.exe server

To run the server as the "games" user:

    tssm set UT2004 ObjectName games password

To configure the server to log to a file:

    tssm set UT2004 AppStdout c:\games\ut2004\service.log

To restrict the server to a single CPU:

    tssm set UT2004 AppAffinity 0

To remove the server:

    tssm remove UT2004 confirm

To find out the service name of a service with a display name:

    tssm get "Background Intelligent Transfer Service" Name


Building TSSM from source
-------------------------
TSSM is known to compile with Visual Studio 2008 and later.  Older Visual
Studio releases may or may not work if you install an appropriate SDK and
edit the tssm.vcproj and tssm.sln files to set a lower version number.
They are known not to work with default settings.

TSSM will also compile with Visual Studio 2010 but the resulting executable
...
registry value for AppDirectory confused TSSM.
...
Thanks to Rob Sharp for pointing out that TSSM did not respect the
...
Thanks to Gerald Haider for noticing that installing a service with TSSM in a
...
Licence
-------
TSSM is public domain.  You may unconditionally use it and/or its source code 
